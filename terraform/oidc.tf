...
data "aws_iam_policy_document" "github_actions_frontend" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.web_app_account.account_id}:parameter/xomcloud/api/*",
      "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.web_app_account.account_id}:parameter/xomcloud/soundcloud/*",
    ]
  }
